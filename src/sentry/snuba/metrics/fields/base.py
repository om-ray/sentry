...
def _get_known_entity_of_metric_mri(metric_mri: str) -> Optional[EntityKey]:
    # ToDo(ahmed): Add an abstraction that returns relevant data based on MetricPathKey without repeating code
    try:
...
def _get_entity_of_metric_mri(
    projects: Sequence[Project], metric_mri: str, use_case_id: MetricPathKey
) -> EntityKey:
...
class MetricObject(MetricObjectDefinition, ABC):
    ...
    @abstractmethod
    def generate_filter_snql_conditions(self, org_id: int, use_case_id: MetricPathKey) -> Function:
        raise NotImplementedError

    @abstractmethod
    def generate_metric_ids(
        self, projects: Sequence[Project], use_case_id: MetricPathKey
    ) -> Set[int]:
        raise NotImplementedError
...
class RawOp(MetricOperation):
    ...
    def generate_snql_function(
        ...
        if use_case_id is MetricPathKey.PERFORMANCE:
...
class DerivedOp(DerivedOpDefinition, MetricOperation):
    ...
    def generate_snql_function(
        ...
        kwargs: MutableMapping[str, Union[float, int, str, MetricPathKey, Function]] = {}
...
class MetricExpression(MetricExpressionDefinition, MetricExpressionBase):
    ...
    def get_entity(self, projects: Sequence[Project], use_case_id: MetricPathKey) -> MetricEntity:
...
class SingularEntityDerivedMetric(DerivedMetricExpression):
    ...
    def __recursively_get_all_entities_in_derived_metric_dependency_tree(
        cls, derived_metric_mri: str, projects: Sequence[Project], use_case_id: MetricPathKey
    ) -> Set[MetricEntity]:
        """
    ...
    def __recursively_generate_metric_ids(
        cls, org_id: int, derived_metric_mri: str, use_case_id: MetricPathKey
    ) -> Set[int]:
        """
...
class CompositeEntityDerivedMetric(DerivedMetricExpression):
    ...
    def naively_generate_singular_entity_constituents(self, use_case_id: MetricPathKey) -> Set[str]:
