...
class MetricsQuery(MetricsQueryValidationRunner):
    ...
    def validate_granularity(self) -> None:
        ...
        if (
            self.use_case_key == MetricPathKey.PERFORMANCE
    ...
    def validate_interval(self) -> None:
        if self.interval is not None:
            if self.use_case_key == MetricPathKey.RELEASE_HEALTH or (
                self.use_case_key == MetricPathKey.PERFORMANCE and not self.include_series
            ):
