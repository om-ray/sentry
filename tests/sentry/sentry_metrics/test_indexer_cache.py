...
def test_cache(use_case_id: str) -> None:
    cache.clear()
    assert indexer_cache.get(f"{use_case_id}:100:blah", use_case_id) is None
    indexer_cache.set(f"{use_case_id}:100:blah", 1, use_case_id)
    assert indexer_cache.get(f"{use_case_id}:100:blah", use_case_id) == 1

    indexer_cache.delete(f"{use_case_id}:100:blah", use_case_id)
    assert indexer_cache.get(f"{use_case_id}:100:blah", use_case_id) is None
...
def test_cache_many(use_case_id: str) -> None:
    ...
    assert indexer_cache.get_many(list(values.keys())) == {
    ...
    assert indexer_cache.get_many(list(values.keys()), use_case_id) == {
        f"{use_case_id}:100:hello": None,
        f"{use_case_id}:100:bye": None,
    }
